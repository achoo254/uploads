...
constructor(private http: HttpClient) { }
  ...
  GetUserInfoByToken(token: string){
    return this.http.get(
      'http://localhost:8081/userInfo/get/token/'+token
    );
  }
  ...
  getUserInfoByTokenAndRoles(token: string, roles: string){
    return this.http.get(
      'http://localhost:8081/userInfo/get/token/' + token + '/roles/' + roles
    );
  }
}
